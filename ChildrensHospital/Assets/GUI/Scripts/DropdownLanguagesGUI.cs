...
public class DropdownLanguagesGUI : MonoBehaviour {
    ...
    private void DrawDropDownList()
    {
        for (int i = 0; i < maxDropdownSize; i++)
        {
            ...
            if (startList + i < LanguageSerializer.Instance.languages.Count)
            {
                // Populates the fields of the menu, up to the maximum dropdown size
                if (GUI.Button(new Rect(scaledResolutionWidth - buttonSize.x - 25, 25 + buttonSize.y * (i + 1) * yPosition, buttonSize.x, buttonSize.y), LanguageSerializer.Instance.languages[startList + i], "Dropdown"))
                {
                    ...
                    LanguageUsed.Instance.CurrentLanguage = i + startList;
