...
public class VolumeTracker {
    ...
    public string TrunkLeft { get { return (trunkLeft * -1.0f).ToString("G8"); } }
    public string TrunkRight { get { return trunkRight.ToString("G8"); } }
    public string TrunkForward { get { return trunkForward.ToString("G8"); } }
    ...
    public void DebugExtremas()
    {
        ...
        using (StreamWriter file = new StreamWriter(path + @"\Extremas.csv"))
        {
            if (lines.Count == 0)
            {
                ...
                + "LowerY, MiddleY, LeftYReach, RightYReach, TrunkLeft, TrunkRight, TrunkForward, CalibrationMatrix, XOffset, YOffset, ZOffset");
            }
